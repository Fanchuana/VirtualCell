...
CELL=${2:-"jurkat"}
STAGE=${3:-"train"}
CONFIG_NAME=${4:-"vae_freeze_ourloss"}
GPU_ID=${5:-0}
...
# --- 3. 日志目录配置 ---
LOG_DIR="${PROJECT_PATH}/experiment/${EXP_NAME}/scripts/output/${TIMESTAMP}_${CONFIG_NAME}/${TASK}_${CELL}"
mkdir -p "${LOG_DIR}"
...
python -m state tx train \
    "data.kwargs.toml_config_path='${PROJECT_PATH}/config/${TASK}_${CELL}.toml'" \
    ...
    "model.kwargs.vae_kwargs.model_path='${PROJECT_PATH}/model_weight/VAE_${TASK}_${CELL}/model_seed=0_step=199999.pt'" \
    "wandb.tags=['replogle_run_${TASK}_${CELL}']" \
    "use_wandb=False" \
    "output_dir='${PROJECT_PATH}/experiment/${EXP_NAME}/model_output/${TASK}_${CELL}'" \
    "name='${CONFIG_NAME}'"
