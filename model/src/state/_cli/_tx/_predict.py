...
def run_tx_predict(args: ap.ArgumentParser):
    ...
    output_path_DE_pred = os.path.join(results_dir, "DE_pred.csv")
    output_path_DE_gt = os.path.join(results_dir, "DE_gt.csv")
    output_path_direction_pred = os.path.join(results_dir, "direction_pred.csv")
    output_path_direction_gt = os.path.join(results_dir, "direction_gt.csv")
    logger.info(f"Output CSVs will be saved to:\n  {output_path_DE_pred}\n  {output_path_DE_gt}")
    ...
    # [TODO] 在这里注册我们自己的类
    elif model_class_name.lower() in ["our_state"]:
        from ...tx.models.our_state_transition import OurStateTransitionPerturbationModel

        ModelClass = OurStateTransitionPerturbationModel
    
    elif model_class_name.lower() in ["vae_transition"]:
        from ...tx.models.vae_transition import VAETransitionPerturbationModel

        ModelClass = VAETransitionPerturbationModel
