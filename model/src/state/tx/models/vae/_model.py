...
class FeedForward(nn.Module):
    def __init__(
        ...
        self.fc1 = nn.Linear(dim, d_ffn, bias=False)
        self.fc2 = nn.Linear(d_ffn, dim, bias=False)
        self.fc3 = nn.Linear(dim, d_ffn, bias=False)
    
    def forward(self, x):
        return self.fc2(F.silu(self.fc1(x)) * self.fc3(x))


class MultiLabelModelBase(nn.Module):
    """
    实现多分类任务的Decoder Model
    """
    def __init__(
        self, 
        input_dim: int = 696,  # 696是transf的默认维度
        hidden_dim: int = 2560, 
        ...
        self.final_layer = nn.Linear(input_dim, num_classes, bias=False) 

    def forward(self, x):
        decode_out = self.ffn(x)
        decode_out = F.silu(decode_out)
        decode_out = self.ffn_norm(decode_out)
        decode_out = self.dropout(decode_out)
        logits = self.final_layer(decode_out)
        return logits
