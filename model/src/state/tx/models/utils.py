...
def load_pretrained_vae(
    ...
    hidden_dim: int = 128,
    freeze: bool = True,
    device: str = "cpu",    # 在加载时默认使用 CPU，让 PL 稍后统一管理设备
    **kwargs,
) -> nn.Module:
    """
    Args:
        ...
        freeze: 是否冻结参数 (设置 requires_grad=False 并设为 eval 模式)
    Returns:
        加载好的 VAE 模型实例
    """
    print(f"Loading pretrained VAE...")
    vae_model = VAE(
        num_genes=input_dim,
    ...
    # # 处理可能存在的 key 不匹配问题 (例如有些权重保存时带 'module.' 前缀)
