...
class OurStateTransitionPerturbationModel(PerturbationModel):
    """
    This model:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if loss_name == "energy":
            ...
            self.loss_fn = SamplesLoss(loss="sinkhorn", blur=blur, backend="multiscale")
        elif loss_name == "ba":
        ...
        print("BA LOSS BUILT:", hasattr(self.loss_fn, "loss_dict"))
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int, padded=True) -> torch.Tensor:
        ...
        # log our ba loss
        if hasattr(self.loss_fn, "loss_dict"):
            ba_loss_dict = self.loss_fn.loss_dict
            self.log_dict(
                ba_loss_dict, 
                prog_bar=True,   # 进度条显示 l_sd, l_mean, l_cov
                on_step=True,    # 确保写入 metrics.csv 的每一行
                on_epoch=False
            )
